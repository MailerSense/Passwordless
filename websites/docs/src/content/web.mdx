...
| :------------------- | :--------------- | :------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `analyticsHost`      | `string`         | —       | **Required.** URL of your Passwordless analytics instance (e.g., `https://rybbit.yourdomain.com/api`).                                                                        |
| `siteId`             | `string \| number` | —       | **Required.** The Site ID for your website obtained from your Passwordless instance.                                                                                        |
...
If a page's path (`window.location.pathname`) matches any pattern in the `skipPatterns` array, the pageview event (and any associated data) for that path will **not** be sent to Passwordless.

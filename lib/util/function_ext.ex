defmodule Util.FunctionExt do
  @moduledoc false
  @doc false
  defmacro __using__(_) do
    quote do
      import Util.FunctionDecorator, only: [deco: 2]
    end
  end
  ...
  defmacro pipe_result(defun, pipe) do
    quote bind_quoted: [pipe: Macro.escape(pipe), defun: defun] do
      Deco.update_do(defun, fn body -> {:|>, [], [body, pipe]} end)
    end
  end
  ...
  def update_do({_def, _, _} = expr, fun) do
  ...
  defmacro around(defun, wrapper) do
    quote bind_quoted: [wrapper: Macro.escape(wrapper), defun: defun] do
      ...
      call_wrapper = Util.FunctionDecorator.add_args(:prepend, wrapper, [ref] ++ args)

defmodule Util.DurationFormatter do
  @moduledoc """
  ...
  defp do_format([{unit, value} | rest], str, locale) do
    ...
    unit_with_value =
      locale
      |> Translator.translate_plural("units", "%{count} #{unit}", "%{count} #{unit}s", count)
  ...
  defp deconstruct(duration) do
    micros = duration |> Duration.to_microseconds() |> abs()
  ...
  defp deconstruct({0, micro}, components) do
    ...
    if millis >= 1 do
      deconstruct({0, 0}, [{:millisecond, millis} | components])
    else
      deconstruct({0, 0}, [{:microsecond, micro} | components])
    end
  end
end

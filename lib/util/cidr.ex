defmodule Util.CIDR do
  ...
  def match(%__MODULE__{first: {a, b, c, d}, last: {e, f, g, h}}, {i, j, k, l} = address) do
  ...
  def match(
        %__MODULE__{first: {a, b, c, d, e, f, g, h}, last: {i, j, k, l, m, n, o, p}},
  ...
  def match(_cidr, _address), do: {:error, "Argument must be a binary or IP tuple of the same protocol"}
  ...
  ### Examples
       ...
       %__MODULE__{first: {10, 0, 0, 0}, hosts: 256, last: {10, 0, 0, 255}, mask: 24}
  ...
  """
  def subnet?(%__MODULE__{mask: mask_a}, %__MODULE__{mask: mask_b}) when mask_a < mask_b do
    false
  end
  ...
  """
  def supernet?(%__MODULE__{mask: mask_a}, %__MODULE__{mask: mask_b}) when mask_a > mask_b do
    false
  end
  ...
  ### Examples
         ...
         [%__MODULE__{first: {192, 168, 0, 0}, hosts: 128, last: {192, 168, 0, 127}, mask: 25},
          %__MODULE__{first: {192, 168, 0, 128}, hosts: 128, last: {192, 168, 0, 255}, mask: 25}]

  """
  def split(%__MODULE__{mask: mask}, new_mask) when mask > new_mask do
    {:error, "New mask must be larger than existing cidr"}
  end

  def split(%__MODULE__{first: {_, _, _, _}} = cidr, new_mask) do
    x = tuple2number(cidr.first, 32 - cidr.mask)
    split(x, new_mask, cidr.mask, :ipv4)
  end

  def split(%__MODULE__{first: {_, _, _, _, _, _, _, _}} = cidr, new_mask) do
    x = tuple2number(cidr.first, 128 - cidr.mask)
    split(x, new_mask, cidr.mask, :ipv6)
  end

  defp split(start, new_mask, old_mask, afi) do
    n = round(:math.pow(2, new_mask - old_mask) - 1)
    ...
    Stream.map(0..n, fn x ->
      ...
      %__MODULE__{first: first, last: last, mask: new_mask, hosts: step}
    end)
  end
  ...
  def parse(string) when is_bitstring(string) do
    [address | mask] = String.split(string, "/")
  ...
  defp create(first, last, mask, hosts) do
    %__MODULE__{first: first, last: last, mask: mask, hosts: hosts}
  end
  ...
  ## Examples
         ...
         [
           %__MODULE__{first: {192, 168, 1, 0}, hosts: 256, last: {192, 168, 1, 255}, mask: 24},
           %__MODULE__{first: {192, 168, 2, 0}, hosts: 1, last: {192, 168, 2, 0}, mask: 32}
         ]
  """
  def parse_range(string) when is_bitstring(string) do
    ...
    case length(ips) do
      # single ip found, use normal `parse\1` function
      1 -> parse(string)
      # found two values, continue
      2 -> parse_range_impl(ips)
      _ -> {:error, :einval}
    end
  end
  ...
  defp parse_range_impl([a, b]) when is_bitstring(a) and is_bitstring(b) do
    case {parse(a), parse(b)} do
      ...
      {%__MODULE__{mask: 32, first: first_a}, %__MODULE__{mask: 32, first: first_b}}
  ...
  defp range_reduce_inner(lower, higher, step, acc) do
    ...
    if outer_test && !inner_test do
      range_reduce_inner(lower, higher, step + 1, acc)
    else
      {acc ++ [{lower, 32 - step}], lower + (1 <<< step)}
    end
  end
end

defmodule LiveToast do
  ...
  def toast_class_fn(assigns) do
    [
      # base classes
      "group/toast z-100 pointer-events-auto relative w-full items-center justify-between origin-center overflow-hidden rounded-lg p-4 shadow-4 col-start-1 col-end-1 row-start-1 row-end-2",
      # start hidden if javascript is enabled
      "[@media(scripting:enabled)]:opacity-0 [@media(scripting:enabled){[data-phx-main]_&}]:opacity-100",
      # used to hide the disconnected flashes
      if(assigns[:rest][:hidden], do: "hidden", else: "flex"),
      # override styles per severity
      assigns[:kind] == :info &&
        "text-success-900 dark:text-white bg-success-100 dark:bg-success-600 border border-success-200 dark:border-success-700",
      assigns[:kind] == :error &&
        "text-danger-700 dark:text-danger-200 bg-danger-100 dark:bg-danger-800 border border-danger-200 dark:border-danger-700"
    ]
  end

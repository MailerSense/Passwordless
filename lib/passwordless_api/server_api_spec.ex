defmodule PasswordlessApi.ServerApiSpec do
  @moduledoc """
  ...
  def spec do
    open_api = %OpenApi{
      ...
      info: %Info{
        title: "Passwordless Server API",
        version: "0.1.0"
      },
      components: %Components{
        securitySchemes: %{
          "authorization" => %SecurityScheme{type: "http", name: "Authorization", scheme: "bearer", in: "header"}
        },
        responses: %{
      ...
      paths:
        Router
        |> Paths.from_router()
        |> Enum.filter(fn {path, _item} -> String.starts_with?(path, "/api/server") end)
        |> Map.new()
    }

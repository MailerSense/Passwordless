defmodule TypedStruct.Cast do
  @moduledoc """
  TypedStruct.Cast is a TypedStruct plugin that provides casting of values to the appropriate field type.
  """
  ...
  def field(name, type, opts, env) do
    quote bind_quoted: [name: name, type: Macro.escape(type), opts: Macro.escape(opts), env: Macro.escape(env)] do
      TypedStruct.Cast.__field__(name, type, opts, env)
    end
  end
  ...
  def __field__(name, _type, opts, %Macro.Env{module: mod}) do
    ...
    unless Keyword.get(opts, :mappable?, true) do
  ...
  def after_definition(_opts) do
    quote unquote: false do
      def __defaults__, do: Enum.reverse(@default_apply_fields)
      ...
      def new(attrs), do: TypedStruct.Cast.do_new(__ENV__.module, attrs)
      ...
      def from(base_struct, attrs), do: TypedStruct.Cast.do_from(__ENV__.module, base_struct, attrs)
  ...
  * Any cast error or missing required field will result in a `raise`
  ...
  def do_new(mod, attrs) when not is_struct(attrs) and is_map(attrs) do
    ...
    |> TypedStruct.Cast.apply_defaults(mod.__defaults__())
  ...
  def do_from(mod, base_struct, attrs) when is_struct(base_struct) do
    ...
    TypedStruct.Cast.do_new(mod, attrs)
  end

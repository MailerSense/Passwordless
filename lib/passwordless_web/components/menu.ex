defmodule PasswordlessWeb.Components.Menu do
  @moduledoc false
  ...
  def vertical_menu_item(%{menu_items: _} = assigns) do
    assigns =
      assigns
      |> assign_new(:submenu_id, fn -> "submenu_#{:rand.uniform(10_000_000) + 1}" end)
      |> assign_new(:icon_id, fn -> "icon_#{:rand.uniform(10_000_000) + 1}" end)
  ...
  defp menu_icon(assigns) do
    ~H"""
    <.icon :if={is_atom(@icon)} name={@icon} class={menu_icon_classes(@is_active)} />
  ...
  defp menu_icon_classes(is_active),
    do: "pc-vertical-menu-item__icon pc-vertical-menu-item__icon--#{if is_active, do: "active", else: "inactive"}"

  defp menu_item_base_classes, do: "pc-vertical-menu-item"

  # Active state
  defp menu_item_classes(page, page), do: "#{menu_item_base_classes()} group pc-vertical-menu-item--active"
  ...
  defp js_attributes("container", "alpine_js", %{name: name, current_page: current_page, menu_items: menu_items}) do
    %{
  ...
  defp js_attributes("submenu", "alpine_js", %{name: name, current_page: current_page, menu_items: menu_items}) do
    %{
      "x-show": "open",
  ...
  defp js_attributes("submenu", "live_view_js", %{name: name, current_page: current_page, menu_items: menu_items}) do

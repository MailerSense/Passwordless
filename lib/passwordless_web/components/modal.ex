defmodule PasswordlessWeb.Components.Modal do
  @moduledoc false
  ...
  attr :badge, :string, default: nil, doc: "what"
  attr :class, :string, default: nil, doc: "modal class"
  attr :title, :string, default: nil, doc: "modal title"
  attr :subtitle, :string, default: nil, doc: "modal subtitle"

  attr :variant, :string,
    default: "info",
    values: ["info", "large", "delete", "main"],
    doc: "modal max width"
  ...
  attr :on_cancel, JS,
    ...
    doc: "a JS function to execute when the modal is closed. Defaults to pushing close_modal event"
  ...
  def modal(assigns) do
    assigns =
      assign(assigns, :classes, get_classes(assigns))
      ...
      >
        ...
        >
          ...
          <div
            :if={not @hide_header}
            class={
              if @variant == "info", do: "pc-modal__header", else: "pc-modal__header-#{@variant}"
            }
          >
            <div class={
              if @variant == "info",
                do: "pc-modal__header__container",
                else: "pc-modal__header__container-#{@variant}"
            }>
              <div class="pc-modal__header__headline">
                <span class={
                  if @variant == "info",
                    do: "pc-modal__header__text",
                    else: "pc-modal__header__text-#{@variant}"
                }>
                  {@title}
                </span>
                <span :if={@subtitle} class="pc-modal__header__subtext">
                  {@subtitle}
                </span>
              </div>
              <%= unless @hide_close_button do %>
                ...
                >
                  ...
                  <Icon.icon name="remix-close-line" class="pc-modal__header__close-svg" />
                </button>
              <% end %>
            </div>
          </div>
          ...
          <div class={
            if @variant == "info",
              do: "pc-modal__content",
              else: "pc-modal__content-#{@variant}"
          }>
  ...
  def show_modal(js \\ %JS{}, id) do
    ...
    |> JS.show(
      ...
      time: 150,
      transition:
        {"transition-all transform ease-out duration-150", "opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95",

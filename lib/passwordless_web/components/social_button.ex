defmodule PasswordlessWeb.Components.SocialButton do
  @moduledoc false
  ...
  attr(:color, :string, values: ["primary", "info", "success", "warning", "danger"])
  ...
  attr(:logo, :string,
    required: true,
    values: ["google", "github", "passwordless"]
  )

  attr(:variant, :string, default: "outline", values: ["outline"])
  ...
  defp social_icon(%{provider: "google"} = assigns) do
    ~H"""
    <svg class="w-6 h-6" viewBox="0 0 19 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
  ...
  defp social_icon(%{provider: "passwordless"} = assigns) do
    ~H"""
    <.icon name="remix-mail-open-line" class="w-6 h-6" />
    """
  end
end

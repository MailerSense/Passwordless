defmodule PasswordlessWeb.App.ProjectLive.Index do
  ...
  defp apply_action(socket, :delete, _params) do
    assign(socket,
      ...
      page_subtitle: gettext("Are you sure you want to delete this project?")
    )
  end
  ...
  defp assign_projects(socket, params) do
    case socket.assigns[:current_org] do
      %Org{} = org ->
        query = Project.get_by_org(org)
        {projects, meta} = DataTable.search(query, params, @data_table_opts)

defmodule PasswordlessWeb.Admin.ItemActions.SoftDelete do
  @moduledoc false
  ...
  @impl Backpex.ItemAction
  def label(_assigns, _item), do: Backpex.translate("Soft Delete")

  @impl Backpex.ItemAction
  def confirm_label(_assigns), do: Backpex.translate("Soft Delete")
  ...
  def confirm(assigns) do
    ...
    if count > 1 do
      Backpex.translate(
        {"Do you want to soft delete these %{count} %{resources}?", %{count: count, resources: assigns.plural_name}}
      )
    else
      Backpex.translate({"Do you want to soft delete this %{resource}?", %{resource: assigns.singular_name}})
    end
  end
      ...
      try do
        {:ok, _items} =
          Repo.with_soft_deleted(fn ->
            Backpex.Resource.update_all(items, [set: [deleted_at: datetime]], "updated", socket.assigns.live_resource)
          end)
        ...
        error ->
          Logger.error("An error occurred while soft deleting the resource: #{inspect(error)}")
  ...
  defp success_message(assigns, [_item]) do
    Backpex.translate({"%{resource} has been soft deleted successfully.", %{resource: assigns.singular_name}})
  end

  defp success_message(assigns, items) do
    Backpex.translate(
      {"%{count} %{resources} have been soft deleted successfully.",
  ...
  defp error_message(assigns, _error, [_item]) do
    Backpex.translate({"An error occurred while soft deleting the %{resource}!", %{resource: assigns.singular_name}})
  end

  defp error_message(assigns, _error, items) do
    Backpex.translate(
      {"An error occurred while soft deleting %{count} %{resources}!",
       %{resources: assigns.plural_name, count: Enum.count(items)}}
    )
  end
end

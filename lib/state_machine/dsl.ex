      ...
      quote do
        if Keyword.has_key?(unquote(opts), :ecto_type) do
          raise CompileError,
            ...
            line: __ENV__.line,
            description:
              "Option `ecto_type` is deprecated, use `state_type` and `event_type` to generate EctoType implementations respectively"
        end
    ...
    quote do
      unless Module.get_attribute(__MODULE__, :in_defmachine) do
        raise CompileError,
          file: __ENV__.file,
          line: __ENV__.line,
          description: "Calling `state` outside of state machine definition"
      end
      ...
      quote do
        unless Module.get_attribute(__MODULE__, :in_defmachine) do
          raise CompileError,
            file: __ENV__.file,
            line: __ENV__.line,
            description: "Calling `event` outside of state machine definition"
        end
    ...
    quote do
      unless Module.get_attribute(__MODULE__, :in_event) do
        raise CompileError,
          file: __ENV__.file,
          line: __ENV__.line,
          description: "Calling `transition` outside of `event` block"
      end
